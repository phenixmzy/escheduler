...
public class HadoopUtils implements Closeable {
                        ...
                        if (resUploadType == ResUploadType.HDFS) {
                            ...
                            if (defaultFS.startsWith("file")) {
                                ...
                                if (StringUtils.isNotBlank(defaultFSProp)) {
                                    ...
                                    fsRelatedProps.entrySet().stream().forEach(
                                ...
                                logger.info("get property:{} -> {}, from core-site.xml hdfs-site.xml ", FS_DEFAULTFS,
                                    defaultFS);
                            }

                            if (fs == null) {
                                if (StringUtils.isNotEmpty(hdfsUser)) {
                                    //UserGroupInformation ugi = UserGroupInformation.createProxyUser(hdfsUser,
     ...
     * @return {@link FileStatus}
     */
    public Long getFileSize(String filePath, String fileName) throws Exception {
        Path path = new Path(filePath);
        Long len = 0L;
        try {
