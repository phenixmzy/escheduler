...
public class ProcessDefinitionMapperProvider {
    ...
    public String queryDefineListPaging(Map<String, Object> parameter) {
        return new SQL() {{
            SELECT("td.*,sc.schedule_release_state,sc.crontab,sc.warning_group_id,"
                + "case when sc.warning_type= 0 then '都不发'\n"
                + " when sc.warning_type= 1  then '成功发'\n"
                + " when sc.warning_type= 2 then '失败发'\n"
                + " when  sc.warning_type= 3 then '成功或失败都发'\n"
                + " else null   end as  warning_type   ");
            FROM(TABLE_NAME + " td");
            LEFT_OUTER_JOIN(" (select process_definition_id,release_state as schedule_release_state,  crontab,warning_group_id,warning_type from `t_escheduler_schedules` " +
            ...
            Object releaseState = parameter.get("releaseState");
            if(releaseState != null ){
                WHERE( " sc.schedule_release_state= #{releaseState} ");
            }
