        ...
        return new SQL() {
            {
                ...
                WHERE("process_definition_id=#{processDefinitionId} ");
                if(parameter.get("startTime") != null && parameter.get("endTime") != null){
                    if("last1Hour".equals(parameter.get("cycle").toString())
                            || "last2Hour".equals(parameter.get("cycle").toString())
                            || "last3Hour".equals(parameter.get("cycle").toString())
                    ){
                        WHERE("schedule_time between #{startTime} and #{endTime}");
                    } else {
                        WHERE("FROM_UNIXTIME(UNIX_TIMESTAMP(schedule_time) + 3600*8) between #{startTime} and #{endTime}");
                    }
                }
        ...
        return new SQL() {
            {
                ...
                if(parameter.get("startTime") != null && parameter.get("endTime") != null){
                    if("last1Hour".equals(parameter.get("cycle").toString())
                        ...
                        WHERE("FROM_UNIXTIME(UNIX_TIMESTAMP(start_time) + 3600*8) between #{startTime} and #{endTime}");
                    }
        ...
        return new SQL() {
            {
                ...
                if(parameter.get("startTime") != null && parameter.get("endTime") != null
                    ...
                    if("last1Hour".equals(parameter.get("cycle").toString())
                        ...
                        WHERE("process_definition_id=#{processDefinitionId} and (FROM_UNIXTIME(UNIX_TIMESTAMP(schedule_time) + 3600*8) between #{startTime} and #{endTime} " +
                                "or FROM_UNIXTIME(UNIX_TIMESTAMP(start_time) + 3600*8) between #{startTime} and #{endTime})");
                    }
                }
                WHERE("`state` in (" + strStates.toString() + ")");
                ORDER_BY("start_time desc limit 1");
