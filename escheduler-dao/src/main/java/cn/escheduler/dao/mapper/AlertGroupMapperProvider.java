...
public class AlertGroupMapperProvider {
        ...
        return new SQL() {
            {
                ...
                VALUES("`user_id`", "#{alertGroup.userId}");
                VALUES("`group_name`", "#{alertGroup.groupName}");
                VALUES("`group_type`", "#{alertGroup.groupType}");
        ...
        return new SQL() {
            {
                ...
                SET("`user_id`=#{alertGroup.userId}");
                SET("`group_name`=#{alertGroup.groupName}");
                SET("`group_type`=#{alertGroup.groupType}");
    ...
    /**
     * query alertGroup and userNames
     * @param parameter
     * @return
     */
    public String queryAlertGroupUserNamesList(Map<String, Object> parameter) {
        return new SQL() {
            {
                SELECT("g.id,g.group_name, uk.user_names");
                FROM(TABLE_NAME + " g ");
                LEFT_OUTER_JOIN(" (SELECT GROUP_CONCAT(u.user_name SEPARATOR ',') user_names,rel.alertgroup_id FROM t_escheduler_user u"
                    + "  left join  t_escheduler_relation_user_alertgroup rel on  u.id = rel.user_id where  u.user_type = 1 group by rel.alertgroup_id) uk on g.id =uk.alertgroup_id");
            }
        }.toString();
    }
}
