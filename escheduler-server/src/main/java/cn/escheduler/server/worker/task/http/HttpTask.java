...
public class HttpTask extends AbstractTask {
    ...
    public void handle() throws Exception {
        ...
        try(CloseableHttpClient client = createHttpClient()) {
            try(CloseableHttpResponse response = sendRequest(client)) {
                ...
                logger.info("请求时间: {}, 请求地址: {}, 请求类型: {},  响应时间 : {}毫秒, 响应状态码 : {}, 响应内容 : {}, 异常日志 : {}",
                        DateUtil.date(startTime), httpParameters.getUrl(),httpParameters.getHttpMethod(), costTime, statusCode, body, output);
                }catch (Exception e) {
            ...
            appendMessage(e.toString());
            exitStatusCode = -1;
            logger.error("发送请求地址[" + httpParameters.getUrl() + "] 连接失败："+output, e);
        }
    }
    ...
    protected CloseableHttpResponse sendRequest(CloseableHttpClient client) throws IOException {
        ...
        logger.info("自定义参数：{}",paramsMap.toString());
        logger.info("请求参数：{}",httpParameters.getHttpParams());
        ...
        if(httpParameters.getHttpParams() != null && httpParameters.getHttpParams().size() > 0){
            for (HttpProperty httpProperty: httpParameters.getHttpParams()) {
                ...
                logger.info("组合自定义参数后的请求参数：{}",params);
        ...
        switch (httpParameters.getHttpCheckCondition()) {
            case BODY_CONTAINS:
                if (StringUtils.isEmpty(body) || !body.contains(httpParameters.getCondition())) {
                    appendMessage(httpParameters.getUrl() + " 的响应内容没有 "
            ...
            case BODY_NOT_CONTAINS:
                if (StringUtils.isEmpty(body) || body.contains(httpParameters.getCondition())) {
                    appendMessage(httpParameters.getUrl() + " 的响应内容包含了 "
                            + httpParameters.getCondition());
                    exitStatusCode = -1;
                }
                break;
            case STATUS_CODE_CUSTOM:
                if (!statusCode.equals(httpParameters.getCondition())) {
                    appendMessage(httpParameters.getUrl() + "的响应状态码是: " + statusCode + ", 必须是: " + httpParameters.getCondition());
                    exitStatusCode = -1;
                }
                break;
            default:
                if (!"200".equals(statusCode)) {
                    appendMessage(httpParameters.getUrl() + "的响应状态码是: " + statusCode + ", 必须是: 200");
                    exitStatusCode = -1;
                }
                break;
        }
