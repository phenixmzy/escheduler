...
public class SqlTask extends AbstractTask {
    ...
    public void handle() throws Exception {
        ...
        if (sqlParameters.getDatasource() == 0){
            ...
            if (dataSource != null){
                if(DbType.MONGODB.name().equals(dataSource.getType().name())){
                    BaseDataSource baseDataSource = JSONObject.parseObject(dataSource.getConnectionParams(),MongodbDataSource.class);
                    // 准备好执行SQL和参数实体映射
                    SqlBinds mainSqlBinds = getSqlAndSqlParamsMap(sqlParameters.getSql());
                    executeMongodbSql(baseDataSource,mainSqlBinds);
                } else if(DbType.HBASE.name().equals(dataSource.getType().name())) {
                    BaseDataSource baseDataSource = JSONObject.parseObject(dataSource.getConnectionParams(),HbaseDataSource.class);
                    // 准备好执行SQL和参数实体映射
                    SqlBinds mainSqlBinds = getSqlAndSqlParamsMap(sqlParameters.getSql());
                    executeHbaseSql(baseDataSource,mainSqlBinds);
                } else if(DbType.ES.name().equals(dataSource.getType().name())) {
                    BaseDataSource baseDataSource = JSONObject.parseObject(dataSource.getConnectionParams(),EsDataSource.class);
                    // 准备好执行SQL和参数实体映射
                    SqlBinds mainSqlBinds = getSqlAndSqlParamsMap(sqlParameters.getSql());
                    executeEsSql(baseDataSource,mainSqlBinds);
                } else{
                    Connection con = null;
                    try {
    ...
    private SqlBinds getSqlAndSqlParamsMap(String sql) {
        sql = CommonUtils.replaceDateFormat(sql);
    ...
    public Connection executeFuncAndSql(BaseDataSource baseDataSource,
        ...
        try {
            ...
            try (PreparedStatement  stmt = prepareStatementAndBind(connection, mainSqlBinds)) {
                // 根据sqltype决定是执行查询还是执行更新
                if (sqlParameters.getSqlType() == SqlType.QUERY.ordinal()) {
                    // 查询语句需要转换为jsonarray并插入到alert中以发送
                    ...
                    if(!resultSet.next()){
                        logger.error("数据集为空");
                        throw new RuntimeException("数据集为空");
                    }else{
                    ...
                    logger.info("execute sql result : {}", JSONObject.toJSONString(array, SerializerFeature.WriteMapNullValue));
                    // logger.info("sqlParameters.getShowType : {}",sqlParameters.getShowType());
                    // 作为附件发送
                   /* if (StringUtils.isEmpty(sqlParameters.getShowType())) {
                        logger.info("ShowType为空，不需要发送电子邮件");
                    } else {
                        if (array.size() > 0) {
                            if (StringUtils.isNotEmpty(sqlParameters.getTitle())) {
    ...
    public void executeMongodbSql(BaseDataSource datasource,
                                        SqlBinds mainSqlBinds){
        String[] hostsPorts = CommonUtils.getHostsAndPort(datasource.getAddress());
        MongoUtils mongoUtils = new MongoUtils(hostsPorts[0], hostsPorts[1], datasource.getUser(), datasource.getPassword(), datasource.getDatabase(), null);
        List<Document> result = mongoUtils.getSqlData(datasource.getDatabase(),mainSqlBinds.getSql());
        logger.info("execute sql result size: {}", result.size());
        if(result == null || result.size() == 0){
            logger.error("数据集为空");
            throw new RuntimeException("数据集为空");
        }else{
            logger.info("execute sql result : {}", JSONObject.toJSONString(result, SerializerFeature.WriteMapNullValue));
            exitStatusCode = 0;
        }
    }

    public void executeHbaseSql(BaseDataSource datasource,
                                  SqlBinds mainSqlBinds){
        String[] hostsPorts = CommonUtils.getHostsAndPort(datasource.getAddress());
        HbaseUtils hbaseUtils = new HbaseUtils(hostsPorts[0], hostsPorts[1]);
        Long result = hbaseUtils.getSqlData(mainSqlBinds.getSql());
        if(result == null){
            logger.error("数据集为空");
            throw new RuntimeException("数据集为空");
        }else{
    ...
    public void executeEsSql(BaseDataSource datasource,
                                  SqlBinds mainSqlBinds){
        String[] hostsPorts = CommonUtils.getHostsAndPort(datasource.getAddress());
        EsUtils esUtils = new EsUtils();
        Long result = esUtils.httpCount(hostsPorts[0], hostsPorts[1],datasource.getDatabase(),mainSqlBinds.getSql());
        if(result == null){
            logger.error("数据集为空");
            throw new RuntimeException("数据集为空");
        }else{
