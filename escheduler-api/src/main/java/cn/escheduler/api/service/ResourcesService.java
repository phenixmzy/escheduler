...
public class ResourcesService extends BaseService {
    ...
    /**
     * 将上传到大禹集群的大文件拷贝到easyscheduler的资源目录下
     * @param loginUser
     * @param name   文件名
     * @param desc   描述
     * @param type   文件类型
     * @param filePath 文件路径和文件名
     * @param isDelete  是否删除hdfs上的原文件
     * @return
     */
    @Transactional(value = "TransactionManager", rollbackFor = Exception.class)
    public Result createBigResource(User loginUser,
                                    String name,
                                    String desc,
                                    ResourceType type,
                                    String filePath,
        ...
        if (!StringUtils.equals(fileName, name)) {
            logger.error("upload resource: {} filePath: {} failed.", name, filePath);
            putMsg(result, Status.HDFS_OPERATION_ERROR);
            throw new RuntimeException(String.format("upload resource: %s file: %s failed.", name, filePath));
        }
        Long bigResourceSize = getBigResourceSize(loginUser, name, filePath, type,isDelete);
        // fail upload
        if (bigResourceSize == 0L) {
            logger.error("upload resource: {} filePath: {} failed.", name, filePath);
            putMsg(result, Status.HDFS_OPERATION_ERROR);
            throw new RuntimeException(String.format("upload resource: %s file: %s failed.", name, filePath));
        }
        ...
        resource = new Resource(name, name, desc, loginUser.getId(), type, bigResourceSize, now, now);
    ...
    /**
     * copy file from hdfs  to easyscheduler hdfs and get big resource size
     *
     * @param loginUser
     * @param name
     */
    private Long getBigResourceSize(User loginUser, String name, String filePath, ResourceType type,Boolean isDelete) {
        ...
        try {
            if (HadoopUtils.getInstance().exists(resourcePath)) {
                fileSize = HadoopUtils.getInstance().getFileSize(filePath, name);
                HadoopUtils.getInstance().copy(filePath, hdfsFilename, isDelete, true);
            } else {
                logger.error("{} is not exist", resourcePath);
            }
        } catch (Exception e) {
    ...
    private String getHdfsFileName(ResourceType resourceType, String tenantCode, String hdfsFileName) {
        ...
        logger.info("getHdfsFileName hdfsFileName is {} ", hdfsFileName);
        return hdfsFileName;
    }
