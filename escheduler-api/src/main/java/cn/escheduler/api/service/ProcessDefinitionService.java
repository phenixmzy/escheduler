...
public class ProcessDefinitionService extends BaseDAGService {
     ...
     * @return
     */
    public Map<String, Object> queryProcessDefinitionListPaging(User loginUser, String projectName, String searchVal,
        ...
        Integer releaseStateInt =null;
        if (StringUtils.isNotEmpty(releaseState)) {
            releaseStateInt=Integer.parseInt(releaseState);
        }
        ...
        List<ProcessDefinition> resourceList = processDefineMapper.queryDefineListPaging(project.getId(),
            searchVal, userId, pageInfo.getStart(), pageSize,releaseStateInt);
        List<AlertGroupUserNames> alertGroupUserNames = alertGroupMapper.queryAlertGroupUserNamesList();
        Map<Integer, AlertGroupUserNames> alertGroupUserNamesMap = new HashMap();
        if (alertGroupUserNames != null && alertGroupUserNames.size() > 0) {
            for (AlertGroupUserNames alertGroupUserName : alertGroupUserNames) {
                alertGroupUserNamesMap.put(alertGroupUserName.getId(), alertGroupUserName);
            }
        }
        if (resourceList != null && resourceList.size() > 0) {
            for (ProcessDefinition processDefinition : resourceList) {
                Integer warningGroupId = processDefinition.getWarningGroupId();
                if (alertGroupUserNamesMap.containsKey(warningGroupId)) {
                    AlertGroupUserNames alertGroupUserNamesEveny = alertGroupUserNamesMap.get(warningGroupId);
                    processDefinition.setWarningGroupName(alertGroupUserNamesEveny.getGroupName());
                    processDefinition.setUserNames(alertGroupUserNamesEveny.getUserNames());
                }
            }
        }
        ...
        if (resultStatus == Status.SUCCESS) {
            ...
            if (processDefinition != null) {
                ...
                try(ServletOutputStream out = response.getOutputStream()) {
                    try(BufferedOutputStream buff = new BufferedOutputStream(out)) {
                        buff.write(rowsJson.getBytes("UTF-8"));
                        buff.flush();
                    }catch (IOException e) {
                        logger.error("exportProcessDefinitionById : {}",e);
                    }
                } catch (IOException e) {
                    logger.error("exportProcessDefinitionById : {}",e);
                }
