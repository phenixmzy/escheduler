...
if [ "$command" = "api-server" ]; then
  ...
  LOG_FILE="-Dspring.config.location=conf/application_master.properties,conf/master.properties -Ddruid.mysql.usePingMethod=false -Dserver.port=13526"
...
case $startStop in
  (start)
    ...
    exec_command="$LOG_FILE $ESCHEDULER_OPTS $DEBUG_OPTS -classpath $ESCHEDULER_CONF_DIR:$ESCHEDULER_LIB_JARS $CLASS"
